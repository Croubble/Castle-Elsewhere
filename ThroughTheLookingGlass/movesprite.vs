...
void main()
{
	...
	float xb = max(0,min(1,x));
	float yb = max(0,min(1,y));
	float x_tex = pos.x - movementoffset.x;
	float y_tex = pos.y - movementoffset.y;
	float x_texb =  max(0,min(1,x_tex));
	float y_texb =  max(0,min(1,y_tex));
	...
	float xPos = true_input.x * atlasCoord.z + (1 - true_input.x) * atlasCoord.x;
